...
class Preprocessor(object):
    ...
    def convert_to_json(self, data_directory, subset=True):
        '''
        ...
        if subset:
            for row in dataset[:10]:
                instance_dict['instances'].append(list(row))
        else:
            for row in dataset:
                instance_dict['instances'].append(list(row))

        if subset:
            final_json_filepath = os.path.join(data_directory, 'demo_data.json')
        else:
